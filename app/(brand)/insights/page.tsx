    ...
    <div className="px-6 py-12 sm:px-12">
      <div className="mx-auto max-w-4xl">
        ...
        <section className="mt-12">
          ...
          {signals.avgScoreByPorosity.every((row) => row.count === 0) ? (
            <p className="text-sm text-muted">No scored searches yet.</p>
          ) : (
            <div className="space-y-3">
              {signals.avgScoreByPorosity.map((row) => (
                <div key={row.porosity} className="flex items-center gap-4">
                  <span className="w-36 shrink-0 text-sm text-sand">
                  ...
                  <div className="h-3 flex-1 overflow-hidden rounded-full bg-warm/[0.06]">
                    <div
                      ...
                      style={{ width: `${row.averageScore ?? 0}%` }}
                    />
                  </div>
