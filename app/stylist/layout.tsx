...
import { StylistSessionProvider, useStylistSession } from "@/lib/stylistSession";

const NAV_ITEMS = [
  { href: "/stylist/dashboard", label: "Dashboard", icon: GridIcon },
  { href: "/stylist/account", label: "Account", icon: UserIcon },
];

export default function StylistLayout({ children }: { children: ReactNode }) {
  return (
    <StylistSessionProvider>
...
function Sidebar() {
  ...
  const { account } = useStylistSession();
    ...
    <aside className="sticky top-0 flex h-screen w-16 flex-col border-r border-warm/10 bg-warm/[0.02] sm:w-60">
      <div className="flex items-center gap-2 px-3 py-6 sm:px-6">
        ...
        <span className="hidden font-serif text-lg text-cream sm:inline truncate">
          {account?.display_name ?? "Qoyl Stylist"}
        </span>
      </div>
